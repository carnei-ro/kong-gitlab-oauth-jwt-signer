...
config:
	...
	@curl -s -X POST http://localhost:8001/services/ -d 'name=oauth' -d url=http://localhost:8000
	@curl -s -X POST http://localhost:8001/services/oauth/routes -d 'paths[]=/callback' -d "name=callback"
	@curl -i -X POST http://localhost:8001/services/oauth/plugins -d "name=${NAME}" -d "config.secure_cookies=false" -d "config.http_only_cookies=false" -d "config.callback_uri=/callback"
